...
The `gen_ai.operation.name` SHOULD be `invoke_agent`. Callers do not observe the internal
structure of a remote agentic system, so `invoke_workflow` is not used on this span even
when the remote system is implemented as a workflow.
...
The two are the same operation: which one applies depends on the API shape the framework
exposes, and callers of the agentic system SHOULD NOT be expected to distinguish them.

Nested and outermost invocations are distinguished by span hierarchy - the outermost one
is a child of the `turn` span - not by the operation name.

Examples: LangChain agents, CrewAI agents, LangGraph graphs, CrewAI crews.
