...
The `gen_ai.operation.name` SHOULD be `invoke_agent`, including when the remote system is
...
| [`gen_ai.main_agent.name`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` When available. | string | Human-readable name of the agent, workflow, or other entity that owns the turn. [8] | `customer_support`; `research_assistant` |
...
| [`gen_ai.workflow.name`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` [11] | string | Human-readable name of the GenAI workflow provided by the application. [12] | `multi_agent_rag`; `customer_support_pipeline` |
...
**[8] `gen_ai.main_agent.name`:** The main agent is what the caller of the agentic system talks to, regardless of
...
**[11] `gen_ai.workflow.name`:** When the invoked entity is a workflow and the name is available.

**[12] `gen_ai.workflow.name`:** The workflow name is usually a static, application-unique identifier defined
...
The `gen_ai.operation.name` SHOULD be `invoke_agent` when the invoked entity is an agent
and `invoke_workflow` when it is a workflow, graph, or crew coordinating several agents.

This span is reported at any nesting depth. The outermost invocation is reported as a child
of the `handle_turn` span.

A workflow invocation that covers the whole request SHOULD NOT be reported as a separate
span: the `handle_turn` span represents it and carries `gen_ai.workflow.name`. Workflows
defined by the application and nested inside another invocation, for example a sub-graph
invoked as a node of another graph, SHOULD be reported.

A workflow invocation that is an internal implementation detail of another operation, for
example an agent or tool that spins up a runner to delegate to a sub-agent, SHOULD NOT be
reported.

Framework-specific semantic conventions SHOULD define which operations to report as
workflows and MAY specify heuristics to distinguish them from standalone agent invocations
and internal operations.

Examples of agent invocations: LangChain agents, CrewAI agents, ADK agents.
...
| [`gen_ai.request.choice.count`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` If available, in the request, and !=1. | int | The target number of candidate completions to return. | `3` |
| [`gen_ai.request.seed`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` If applicable and if the request includes a seed. | int | Requests with same seed value more likely to return same result. | `100` |
| [`gen_ai.workflow.name`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Conditionally Required` [9] | string | Human-readable name of the GenAI workflow provided by the application. [10] | `multi_agent_rag`; `customer_support_pipeline` |
| [`gen_ai.request.frequency_penalty`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` | double | The frequency penalty setting for the GenAI request. | `0.1` |
| [`gen_ai.request.max_tokens`](/docs/registry/attributes/gen-ai.md) | ![Development](https://img.shields.io/badge/-development-blue) | `Recommended` | int | The maximum number of tokens the model generates for a request. | `100` |
...
**[9] `gen_ai.workflow.name`:** When the invoked entity is a workflow and the name is available.

**[10] `gen_ai.workflow.name`:** The workflow name is usually a static, application-unique identifier defined
in a framework-specific way.
